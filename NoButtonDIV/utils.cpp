...
// Note: Touchscreen.h is no longer needed
...
void displayLogo(uint16_t color, int displayTime) {
    ...
    tft.setTextSize(2);
    int16_t textWidth = tft.textWidth("BlackWireV1.1", 2);
    ...
    tft.setTextSize(1);
    textWidth = tft.textWidth("by RealKeju", 1);
    textX = screenWidth / 3.5;
    textY += 20;
    tft.setCursor(textX, textY);
    tft.print("by RealKeju");
    ...
    Serial.println("===================================");
    Serial.println("BlackWireV1.1 For CYD              ");
    Serial.println("Developed by: RealKeju             ");
    Serial.println("Version:      1.1.0                ");
    Serial.println("Contact:      realkeju@icloud.com  ");
    Serial.println("GitHub:       github.com/bBlackWire");
...
void terminalSetup() {
    // The touch setup is handled by TFT_eSPI, so this explicit call is not needed.
    // setupTouchscreen();
